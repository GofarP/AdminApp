...
fun ItemCategoriesScreen(
    onNavigateBack: () -> Unit,
    viewModel: ItemCategoriesViewModel= viewModel()
){
    ...
    var itemCategoriesToDelete by remember { mutableStateOf<ItemCategories?>(null) }
    ...
    LaunchedEffect(state.actionSuccess) {
        if (state.actionSuccess) {
            val message = when (currentAction) {
                "insert" -> "Data Item Categories berhasil ditambahkan"
                "update" -> "Data Item Categories berhasil diubah"
                "delete" -> "Data Item Categories berhasil dihapus"
                else -> ""
            }
        ...
        Box(
            ...
            when(val dataStatus=state.dataState){
                is UiState.Idle, is UiState.Loading->{
                    SearchableDataList(
                        items = emptyList<ItemCategories>(),
                        isLoading = true,
                ...
                is UiState.Success->{
                    val filteredItemCategories=dataStatus.data.filter {
                        it.name.contains(searchQuery, ignoreCase = true) ||
                                it.description.contains(searchQuery, ignoreCase = true)
                    }

                    SearchableDataList(
                        items = filteredItemCategories,
                        isLoading = false,
                        searchQuery=searchQuery,
                        onSearchQueryChange = { searchQuery = it },
                        ...
                        listItem = {itemCategories ->
                            ...
                            BaseMasterCard(
                                ...
                                CustomTextField(
                                    ...
                                    placeholder = "Nama Item Categories",
                    ...
                    Column(
                        ...
                        CustomButton(
                            ...
                            modifier = Modifier
                                .width(150.dp)
                                .padding(top = 14.dp),
                            onClick = { viewModel.fetchItemCategories() }
                        )
                    }
                }
            }
    ...
    if(showBottomSheet){
        FormBottomSheet(
            ...
            onSave = {
                viewModel.insertItemCategories(inputName, inputDesc)
            }
        ) {
            CustomTextField(
                value = inputName,
    ...
    itemCategoriesToDelete?.let { item->
        ConfirmDeleteDialog(
            itemName = item.name,
            onDismiss = { itemCategoriesToDelete = null },
            onConfirm = {
                viewModel.deleteItemCategories(item.id)
            }
        )
    }
